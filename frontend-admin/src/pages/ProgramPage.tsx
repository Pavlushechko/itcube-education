...
export function ProgramPage() {
  ...
  const appByGroup = useMemo(() => {
    ...
    for (const a of myApps) {
      // myApps отсортирован desc => первая запись по группе = самая новая
      if (!m.has(a.GroupID)) m.set(a.GroupID, a.Status)
    }
    return m
  }, [myApps])
  ...
  useEffect(() => {
    // проверяем доступ к обучению только для тех групп, где последняя заявка approved
    ...
    for (const g of (data.Groups ?? []) as any[]) {
      ...
      api.listMaterials(g.ID)
        .then(() => {
          setLearnOkByGroup(prev => ({ ...prev, [g.ID]: true }))
        })
        .catch(() => {
          setLearnOkByGroup(prev => ({ ...prev, [g.ID]: false }))
        })
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [data, ident.role, myApps])

  async function apply(groupId: string) {
    const st = appByGroup.get(groupId)
    // если уже есть заявка и она НЕ cancelled — не даём создать ещё одну
    if (st && st !== 'cancelled') return

    const comment = (draftByGroup[groupId] ?? '').trim()

    try {
      const res = await api.createApplication(groupId, comment)
                  ...
                  <div>
                    {alreadyApplied ? (
                      <div>
                        ...
                        {(status === 'submitted' || status === 'in_review') ? (
                          <button
                            onClick={async () => {
                              ...
                              if (!app) {
                                alert('Не найдена заявка для этой группы')
                                return
                              }
                              if (!confirm('Отменить заявку?')) return
                              try {
                                await api.cancelMyApplication(app.ID)
                                await reload()
                              } catch (e: any) {
                        ...
                        {/* rejected -> НИЧЕГО, повторно в эту группу нельзя */}
                      </div>
                    ) : (
                      <div>
                        <div>О себе</div>
                    ...
                    {status === 'approved' && learnOkByGroup[g.ID] === true ? (
                      <div>
                        <Link to={`/learn/group/${g.ID}`}>Перейти в обучение</Link>
                      </div>
                    ) : null}
                  </div>
